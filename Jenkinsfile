pipeline {
	...
	stage("Debugging HashiCorp Vault Credentials") {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-Classroom-Scheduler-WebApp-Deployment.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop clsrmsch | true"
                sh "docker rm clsrmsch | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name clsrmsch -p 5000:80 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service clsrmsch-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"clsrmsch\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
